...
router.get("/:id", async (req: any, res) => {
  ...
  // Lấy tên bác sĩ cho từng medical record
  const vetIds = [
    ...new Set(pet.medicalRecords.map((r) => r.vetId).filter(Boolean)),
  ];
  const vets = await prisma.user.findMany({
    where: { id: { in: vetIds as number[] } },
    select: { id: true, name: true },
  });
  const vetMap = Object.fromEntries(vets.map((v) => [v.id, v]));
  const recordsWithVet = pet.medicalRecords.map((r) => ({
    ...r,
    vet: r.vetId ? vetMap[r.vetId] : null,
  }));

  res.json({ success: true, data: { ...pet, medicalRecords: recordsWithVet } });
});
...
router.post("/:id/vaccinations", async (req: any, res) => {
  const v = await prisma.vaccination.create({
    data: {
      petId: +req.params.id,
      ...req.body,
...
router.get("/:id/medical-records", async (req: any, res) => {
  res.json({
    success: true,
    data: await prisma.medicalRecord.findMany({
      where: { petId: +req.params.id },
      include: { files: true },
      orderBy: { recordDate: "desc" },
    }),
...
router.post("/:id/medical-records", async (req: any, res) => {
  const r = await prisma.medicalRecord.create({
    data: { petId: +req.params.id, vetId: req.user.id, ...req.body },
  });
